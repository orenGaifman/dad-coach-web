...
- [ ] 2. Phase 2: Invitation Validation
  ...
  - [ ] 2.2 Create `src/components/onboarding/InvitationError.tsx` — handles all error states: 404 (invalid), 410 (expired/used), 429 (rate limited with countdown). Each displays appropriate copy per FR-4. Use `/illustrations/error-state.webp` for error display.
  ...
  - [ ] 2.3 Create `src/components/onboarding/WelcomeScreen.tsx` — displays value proposition, inviter name (if available), "Get Started" button. Triggers session creation on click. Use `/illustrations/onboarding-welcome.webp` as hero illustration (father and child on starlit path).
...
- [ ] 3. Phase 3: Wizard Shell and Navigation
  ...
  - [ ] 3.3 Create `src/components/onboarding/OnboardingLayout.tsx` — renders StepIndicator + content slot + navigation footer (Back/Next/Skip buttons). Enforces linear navigation rules. Use app logo from `/logos/dad-coach-logo-icon.webp` in header.
...
- [ ] 4. Phase 4: Father Profile Form
  - [ ] 4.1 Create `src/components/onboarding/ProfileForm.tsx` — form with: display_name, phone_number (with country code selector), email (optional), timezone (with auto-detect pre-fill). Zod schema for client validation. Display `/illustrations/onboarding-father-info.webp` above the form.
...
- [ ] 5. Phase 5: Children Setup
  - [ ] 5.1 Create `src/components/onboarding/ChildrenForm.tsx` — dynamic list of child forms. "Add another child" button (max 8). Remove button per child. Display `/illustrations/onboarding-children.webp` as header illustration.
...
- [ ] 6. Phase 6: Goals and Preferences
  - [ ] 6.1 Create `src/components/onboarding/GoalsSelector.tsx` — predefined goal cards (multi-select 1–5) + custom goal text input (max 100 chars). Visual selection state. Display `/illustrations/onboarding-goals.webp` as header illustration.
...
- [ ] 8. Phase 8: Provisioning and Activation
  - [ ] 8.1 Create `src/hooks/useActivationPolling.ts` — custom hook implementing long-poll loop: fetches activation-status (30s hold), handles status transitions, cleans up on unmount via AbortController.
    - _Requirements: 9.3_
  - [ ] 8.2 Create `src/components/onboarding/ActivationScreen.tsx` — displays WhatsApp deep link button, instructions text, "copy message" fallback, polling status indicator. Use `/illustrations/onboarding-activation.webp` as illustration.
  ...
  - [ ] 8.3 Create `src/components/onboarding/ActivationSuccess.tsx` — success state: "You're connected! 🎉" + "Go to Dashboard" button (navigates to workspace route per WEB-SPEC-008). Use `/illustrations/onboarding-success.webp` and `/illustrations/celebration-confetti.webp`.
    - _Requirements: 9.4_
  ...
  - [ ] 8.4 Create `src/components/onboarding/ActivationFailed.tsx` — failure state: "We didn't receive your message." + retry button. After max retries: "We'll send you a reminder." Use `/illustrations/error-state.webp`.
